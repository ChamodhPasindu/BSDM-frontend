...
@UntilDestroy()
@Component({
  selector: 'app-add-edit-view-employee',
  templateUrl: './add-edit-view-employee.component.html',
  styleUrls: ['./add-edit-view-employee.component.scss'],
})
export class AddEditViewEmployeeComponent
        ...
        next: (res: { roles: IResponse; status: IResponse }) => {
          ...
          if (res.status.body.status === RSP_SUCCESS) {
            if (this.userRole === UserRole.SUPERADMIN) {
              ...
              this.roleList = res.roles.body.content.dropdown.filter(
                (status: Record<string, string>) =>
                  status['code'] !== UserRole.SUPERADMIN,
              );
            }
          }
